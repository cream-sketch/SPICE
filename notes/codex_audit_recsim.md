AUDIT spice_rec_sim.py (faithful SPICE-REC timeline simulator) for correctness BEFORE I trust it. You earlier mandated a conservative copy-engine timeline (fetch is predecessor, no magic overlap; CPU-serve frees PCIe; shadow/protected prefetch insertion; metric useful_on_time_REC_bytes/theoretical_window_bytes). Check:
1. Copy-engine timeline correct & conservative: fetch_fallback demand fetches serialize on copy_free, fetched GPU compute starts AFTER fetch_end (predecessor, no self-overlap)? cpu policies: activation roundtrip on copy engine, then REC prefetch fills ONLY idle window [copy_free, layer_end]?
2. Is spice_rec's gain mechanism faithful: CPU-serve misses (no weight H2D) -> PCIe idle -> prefetch downstream CURRENT-token experts into PROTECTED shadow buffer -> future layers promote on-time completions? No magic (prefetch completion respected via shadow[k]<=clock)?
3. Bug risks: clock/copy_free bookkeeping across layers; shadow promotion timing; exposed_cpu/exposed_fetch accounting; the useful_on_time_frac = useful_bytes/theoretical_window_bytes meaningful?
4. Fairness: all policies same warm cache; spice_rec vs cpu_fiddler differ ONLY in REC prefetch; random_rec/dummy_prefetch isolate prediction-value vs contention?
5. Biggest reason this is still optimistic or wrong, and whether the GO rule (spice_rec beats cpu_fiddler >=15% AND < random_rec) is sound.
