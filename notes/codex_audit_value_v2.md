AUDIT for correctness (I make mistakes, be harsh). This is spice_x_eviction_value.py after adding: (a) --w_rec recency survival term, (b) value_oracle_within ceiling variant, (c) within_mode threading. Prior 2 leakage bugs you found are fixed (transition table B; input_tid shift).

KEY EMPIRICAL FINDING to validate the interpretation of: at 5% residency, hit-rates are LS=0.0666, value_within(soft A, rho0)=0.0802, value(A+B, rho0.5)=0.1144, value_oracle_within(TRUE current-token future-layer experts, rho0,wrec0)=0.0528, Belady=0.3148. So oracle-within (perfect within-token = what SPICE draft predicts) is WORSE than LS, while realizable value (A+transition B) is best.

Questions:
1. Is value_oracle_within implemented correctly as an UPPER BOUND on within-token contribution? (within=1 if e in true_by_layer[cur_tok][j] for j>cur_layer, rho=0, w_rec=0). If correct, is my interpretation valid: "within-token routing prediction is the WRONG signal for cache residency (use-once-then-gone); cross-token transition B drives cache hits"? Or is there a bug making oracle-within artificially bad (e.g. it ignores that those experts ARE imminent and should be kept, so a good policy keeps them too)?
2. Is it fair/correct that value_within (soft A) BEATS value_oracle_within (hard true)? My explanation: soft A also encodes cross-token frequency via Laplace smoothing toward layer marginal, acting partly as LFU. Plausible or bug?
3. Any remaining leakage in w_rec/recency (uses ls_dist from key_layer, past-only) or oracle within (uses true_by_layer of CURRENT token, known)?
4. Is the noisy-OR survival 1-(1-within)(1-cross)(1-recency) sound here?
5. Verdict: trust the oracle-within finding (within-token useless for residency, cross-token is the lever)? And is the realizable value+LS comparison fair/publishable as a hit-rate diagnostic?
Full current script attached.
