SCRUTINIZE this POSITIVE result hard (positive results are where I fool myself). After fairness fixes (warm top-pop cache for ALL policies; three-resource overlap cost region=max(copy,gpu,cpu); fail-fast split), on small Qwen (16 test seq, 2048 tok), token-value selective admission BEATS both fetch_all and cpu_always(Fiddler) at ALL residencies:

5% : fetch_all=80.0  cpu_always=61.2  oracle_admit(r>=2)=68.1  token_admit@0.02=55.8  (H2D: fetch_all 1482, token 139 MB/tok; hits: cpu_always 14833, token 22287)
10%: fetch_all=73.3  cpu_always=57.2  oracle(r>=2)=61.4  token@0.02=51.8
20%: fetch_all=61.4  cpu_always=48.9  oracle(r>=2)=50.6  token@0.02=44.9

token admits a SMALL set (139 MB/tok vs fetch_all 1482) yet gets MORE hits than cpu_always's static-popularity cache.

HARD QUESTIONS (be brutal, find the artifact if any):
1. Is token_admit beating cpu_always REAL or an artifact of: (a) three-resource max cost making CPU-burst the bottleneck so any hit-rate gain shows as TPOT; (b) token churning the warm cache toward better experts while cpu_always is frozen static; (c) oracle mistuned (r_star=2 over-admits -> high copy_t -> looks bad)?
2. The mechanism claim "selective admission caches reused experts cheaper than static popularity" -- is getting 22287 hits at 139MB/tok vs cpu_always 14833 hits at 0MB a fair/meaningful win, given token DOES pay 139MB/tok H2D that cpu_always doesn't? Under three-resource max with low fetch, that copy is hidden -- but is that the optimistic-overlap assumption doing the work? Would a barrier/partial-overlap cost flip it?
3. Why is oracle_admit WORSE than token? If the cost model penalizes fetching, the TRUE optimal admission admits very little -> is token essentially "cpu_always + a few high-value admits", i.e. a marginal improvement over Fiddler, not a category win? Quantify: token@0.02 cpu-served=157905 vs cpu_always 181775 -- only ~13% fewer CPU serves.
4. What is the SINGLE experiment/metric that would prove this is a real residency-investment win and not a cost-model artifact? (e.g. barrier-cost recheck, real wall-clock, matched-H2D oracle.)
5. Verdict: is "future-value-aware admission beats Fiddler+fetch-all" a defensible claim under this proxy, or must I rebuild the cost model / add real prefetch first?
