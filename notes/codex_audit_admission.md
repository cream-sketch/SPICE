AUDIT spice_x_admission.py for correctness before I trust its TPOT verdict (I make mistakes). It implements the 4-policy admission experiment (fetch_all/cpu_always/oracle_admit/token_admit) with your previously-recommended resource-DAG cost model. v1 uses demand-fetch only (no prefetch) to isolate admission. Check:
1. Cost DAG correct: gpu_done=max(t_shared+n_resident*t_gpu, n_fetch*t_fetch)+n_fetch*t_gpu; region=max(gpu_done, cpu_burst(n_cpu)). Demand-fetch as predecessor handled right?
2. oracle future_reuse via occ_ptr monotonic advance with pos+routed.index(e) -- correct future reuse count? Any off-by-one / occ_ptr corruption across keys?
3. cpu_always uses static top-popularity resident, never admits; fetch_all admits all; oracle/token admit per rule then LS-evict. Fair across policies (same capacity, same cost model)?
4. token_value uses realizable within A (j>cur_layer) + transition B (input_tid aligned). No leakage?
5. Is comparing TPOT across policies under this single-stream proxy valid for a RELATIVE verdict (token approaches oracle, beats fetch_all & cpu_always)? Biggest risk that flips the ranking?
